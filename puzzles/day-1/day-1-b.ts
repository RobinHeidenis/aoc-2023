...
export async function day1b(dataPath?: string) {
  ...
  return data.map((line) => {
    ...
    for (let i = 0; i < line.length - 1;) {
      ...
      const possibleMatches = Object.keys(numberMap).filter(key => key.startsWith(substring[0]))
      ...
      const notFoundKey = possibleMatches.every(key => {
        if (substring.startsWith(key)) {
          ...
          for (let numberMapKey in eligibleFlowNumbers) {
            ...
            if (line.substring(i + key.length - 1, i + key.length + 6).startsWith(numberMapKey)) {
