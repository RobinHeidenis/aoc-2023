...
const shootBeam = (
  ...
  takeStep(startingPosition, direction, matrix, energizedTiles);
...
export async function day16b(dataPath?: string) {
  ...
  let maxAmountOfTilesEnergized = 0;

  // Top and bottom rows
  for (let i = 0; i < data[0].length; i++) {
    maxAmountOfTilesEnergized = Math.max(
      maxAmountOfTilesEnergized,
      shootBeam([i, 0], SOUTH, data),
      shootBeam([i, data.length - 1], NORTH, data),
    );
  }
  ...
  for (let i = 0; i < data.length; i++) {
    maxAmountOfTilesEnergized = Math.max(
      maxAmountOfTilesEnergized,
      shootBeam([0, i], EAST, data),
      shootBeam([data[0].length - 1, i], WEST, data),
    );
  }

  return maxAmountOfTilesEnergized;
}
